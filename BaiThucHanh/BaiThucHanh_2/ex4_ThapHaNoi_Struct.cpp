...
{ // Day mot phan tu (loi goi) vao ngan xep
...
{ // Lay mot phan tu (loi goi) o dinh ngan xep de xu ly
    if (empty(s))
        return;
    call_out = s.e[s.top];
    if (s.top == 0)
    { // Lay ra va het, khoi tao ngan xep rong
        delete[] s.e;
        init(s);
    }
    else
...
{ // first_call la loi goi dau tien - chuyen(n, a, b, c);
